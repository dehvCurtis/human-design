...
class ChartExportService {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          ...
          pw.Center(
            child: pw.Text(
              'Generated by Inside Me: Human Design',
              style: pw.TextStyle(
                fontSize: 10,
  ...
  /// Export an AI reading as PDF
  static Future<void> exportReadingAsPdf({
    required HumanDesignChart chart,
    required String aiReadingText,
    required BuildContext context,
  }) async {
    final pdf = pw.Document();
    ...
    final tempDir = await getTemporaryDirectory();
    final sanitizedName = chart.name.replaceAll(RegExp(r'[^\w\s-]'), '');
    final file = File('${tempDir.path}/${sanitizedName}_ai_reading.pdf');
    await file.writeAsBytes(bytes);

    await SharePlus.instance.share(
      ShareParams(
        files: [XFile(file.path)],
        subject: '${chart.name} - AI Chart Reading',
      ),
    );
  }
